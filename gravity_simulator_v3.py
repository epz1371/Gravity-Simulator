...
root.iconbitmap('C:/Users/Essi/Desktop/final_builds/earth.ico')
...
def move(event):
    ...
    #If the current object clicked has the "BALL" tag, we should be allowed to move it vertically.
    if "BALL" in main_canvas.gettags(CURRENT):
        ...
        #Change the coords of the CURRENT object based on the event.y position of the mouse.  Recall ball has size 10.
        main_canvas.coords(CURRENT, x1, event.y, x2, event.y+10)

        #Attemp to not move ball off canvas.  CURRENT[3] is y2 coord
        #Above the top of the screen
        if main_canvas.coords(CURRENT)[3] < 15:
            main_canvas.coords(CURRENT, x1, 5, x2, 15)
        #Below the bottome of the screen
...
def update_height():
    '''Update the height lables for each ball'''
    for i in range (1,5):
...
def step(t):
    ...
    for i in range(1,5):
        ...
        #Get the x coords fore the current ball.  These remain constant
        ...
        if main_canvas.coords(balls['ball_%d' % i])[3] + d <= 415:
            ...
            y3 = main_canvas.coords(balls['ball_%d' % i])[3]
            main_canvas.create_line(x1, y3, x2, y3, tag="DASH")
        else:
            main_canvas.coords(balls['ball_%d' % i], x1, 405, x2, 415)
   
        #Calculate final v value for iteartion
        vf = v + a*t
        #Update velocity values for given time interval.
        velocities['v_%d' % i].delete(0, END)
        velocities['v_%d' % i].insert(0, str(round(-vf, 2)))

        #Add data for step to data dict
        data['data_%d' % i].append((time, 415- main_canvas.coords(balls['ball_%d' % i])[3]))

    #Update heights for given time interval
    update_height()
...
def run():
    ...
    #Balls may start on the ground or at the top of screen so call step() at least once
    step(t_slider.get())

    #Run step() until ALL balls have hit the ground or left screen based of y2 coord[3]
...
def graph():
    ...
    #Colors of the balls coorespsonds to colors of the graph
...
def reset():
    """Erease all "DASH" tags from canvas, set balls back to ground, reset entry fields."""
    global time
    ...
    #Clear each ball...
    for i in range(1,5):
        #Clear and set velocity and accleration
        velocities['v_%d' % i].delete(0, END)
...
line_5 = main_canvas.create_line(400, 0, 400, 415)
